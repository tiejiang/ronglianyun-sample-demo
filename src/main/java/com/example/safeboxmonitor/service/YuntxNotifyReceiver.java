package com.example.safeboxmonitor.service;
...
public class YuntxNotifyReceiver extends ECNotifyReceiver {
   ...
   /* @Override
    public void onReceiveMultiDeviceState(Context ctx, ECMultiDeviceState... multiDevice) {
        super.onReceiveMultiDeviceState(ctx, multiDevice);
        for(int i=0;i<multiDevice.length;i++){
            showToast(multiDevice[i]);
        }
    }*/
...
//        LogUtil.d("ECSDK_Demo.YuntxNotifyReceiver", "onNotificationClicked notifyType " + notifyType + " ,sender " + sender);
    ...
    public void onSoftVersion(Context context, String version , String softDesc, boolean force) {
        ...
        Log.e("aaaa", version + softDesc + force);
...
//                LogUtil.e(TAG, "receiveImp invalid opcode.");
            ...
            switch (optCode) {
                case EVENT_TYPE_CALL:
                    Log.d("TIEJIANG", "receive call event ");
